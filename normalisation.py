...
def compute_normals(depth: np.ndarray, fx: float, fy: float, cx: float, cy: float,
                     smooth_sigma: float = 1.5, eps: float = 1e-6) -> tuple:
    depth_smooth = gaussian_filter(depth, sigma=smooth_sigma) if smooth_sigma > 0 else depth
...
def normals_to_rgb(N: np.ndarray) -> np.ndarray:
    return ((N + 1.0) * 0.5 * 255).astype(np.uint8)
...
def parse_depth_packet(packet: dict) -> tuple:
    ...
    return packet["frame_id"], packet["timestamp"], depth_map.astype(np.float64)
...
def build_geometry_frame(depth_packet: dict, fx: float, fy: float, cx: float, cy: float,
    ...
    P_pad = np.pad(P, ((1, 1), (1, 1), (0, 0)), mode="edge")
    norm_raw = np.linalg.norm(np.cross(
        P_pad[1:-1, 2:, :] - P_pad[1:-1, :-2, :],
        P_pad[2:, 1:-1, :] - P_pad[:-2, 1:-1, :]
    ), axis=-1)
    valid_mask = norm_raw > 1e-8
